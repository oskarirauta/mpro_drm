 ...
 *
 * Rakentaa 128-tavuisen EDID-blokin julkaisemaan laitteen nimen,
 * fyysiset mitat ja preferred timing:n. Antaa userspace-työkaluille
 * (xrandr, edid-decode, kompositorit) tunnistettavan laitteen ilman
 * todellista EDID-piiriä.
 */
...
int mpro_drm__create_edid(struct mpro_drm *mdrm)
{
	...
	const u32 clock_10khz = MPRO_TIMING_CLOCK_KHZ * 0.1;	/* 29.73 MHz */
	const u32 hblank = MPRO_TIMING_HSYNC_OFF + MPRO_TIMING_HSYNC_PULSE + MPRO_TIMING_HBACK;	/* 40 + 48 + 40 */
	const u32 vblank = MPRO_TIMING_VSYNC_OFF + MPRO_TIMING_VSYNC_PULSE + MPRO_TIMING_VBACK;	/* 13 + 3 + 29 */
	...
	edid[16] = 1;		/* week */
	/* year - 1990 = ... */
	edid[17] = (tm.tm_year + 1900) - 1990;
	edid[18] = 1;		/* EDID 1.3 */
	edid[19] = 3;
	edid[20] = 0x80;	/* digital input */
	edid[21] = (wm + 9) / 10;	/* h size cm */
	edid[22] = (hm + 9) / 10;	/* v size cm */
	edid[23] = 120;		/* gamma 2.20 */
	edid[24] = 0x0a;	/* digital, RGB, preferred timing */

	/* 0x19-0x22: chromaticity (sRGB-tyylinen oletus) */
	edid[25] = 0xee;
	edid[26] = 0x91;
	edid[27] = 0xa3;
	edid[28] = 0x54;
	...
	/* 0x26-0x35: standard timings — 0x01 0x01 = ei käytössä */
	for (i = 0x26; i < 0x36; i++)
		edid[i] = 0x01;
	...
	p[11] = (((hsync_off >> 8) & 0x3) << 6) |
	    (((hsync_pulse >> 8) & 0x3) << 4) |
	    (((vsync_off >> 4) & 0x3) << 2) | ((vsync_pulse >> 4) & 0x3);
	p[12] = wm & 0xff;
	p[13] = hm & 0xff;
